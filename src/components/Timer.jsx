...
export default function Timer() {
  ...
  return (
    <section
      className={`bg ${color} cursor-default max-w-screen-md md:mx-auto md:w-full m-2 transition ease-in-out text-white flex flex-col gap-8 justify-evenly items-center h-96  rounded-xl p-6`}
    >
      <h2 className="capitalize text-2xl">{phaseTitle.current}</h2>

      <p className="text-7xl font-semibold text-center">{formattedTime}</p>

      <ToggleTimerBtn started={isActive} onClick={toggleTimer} color={color} />
